...
public class CartController {
	...
	try {
		Integer user_id=getCurrentUserId();
		if(user_id==null) {
			redirectAttributes.addFlashAttribute("succesSMessage","Please login to view your cart items");
			return "redirect:/user/login";
		}
	...
	public String updateCartQty(@RequestParam Integer cartId, @RequestParam int delta,RedirectAttributes redirectAttributes,HttpSession session) {
		try {
			Integer user_id=getCurrentUserId();
			...
			BigDecimal totalAmount=preTotalAmount.subtract(couponDiscount);
			if(coupon!=null) {
				BigDecimal minamount=BigDecimal.valueOf(coupon.getMinAmount());
			
			if(coupon!=null && minamount.compareTo(totalAmount) > 0) {
	...
	public String RemoveCart(@RequestParam("id") int cart_id,RedirectAttributes redirectAttributes,HttpSession session) {
		try {
			Integer user_id=getCurrentUserId();
