...
public class ProductModel {
	...
	public ProductModel(int id, String name, String description, BigDecimal basePrice, BigDecimal discountPer,
			...
			LocalDateTime createAt, BigDecimal finalPer, LocalDateTime updatedAt, boolean is_active,
			int maxDiscountPrice, BigDecimal finalPrice, List<ProductImages> images, List<ProductVariant> variants) {
		super();
		this.id = id;
		this.name = name;
	...
	@Transient // This prevents Hibernate from treating this getter as a persistent property
	public BigDecimal getFinalPer() {
	    BigDecimal productLevelDiscount = this.discountPer != null ? this.discountPer : BigDecimal.ZERO;
	    BigDecimal categoryLevelDiscount = BigDecimal.ZERO;

	    if (productType != null && productType.getCategoryOffers() != null) {
	        Optional<CategoryOffer> activeOffer = productType.getCategoryOffers()
	            .stream()
	            .filter(offer -> offer.getStatus() == OfferStatus.Active)
	            .findFirst();

	        if (activeOffer.isPresent()) {
	            categoryLevelDiscount = activeOffer.get().getCategoryOffer();
	        }

	}
	    return productLevelDiscount.max(categoryLevelDiscount);
	...
	public BigDecimal getFinalPrice() {
		   if (basePrice != null && discountPer != null) {
		        BigDecimal discountAmount = basePrice.multiply(getFinalPer()).divide(BigDecimal.valueOf(100));
