	  ...
	  .then(orderData => {
	    var options = {
		  ...
		  "handler": function (response) {
		         // ✅ Razorpay says payment is successful → verify with backend
		         verifyPayment(response);
		       },
		       "modal": {
		         // ❌ Handles popup close or failure
		         "ondismiss": function () {
		           // Immediate redirect or failure popup
				   fetch("/user/api/payment/mark-failed", {
				      method: 'POST',
				      headers: {
				        'Content-Type': 'application/json'
				      },
				      body: JSON.stringify({ orderId: orderData.id }) // or your internal orderId
				    }).finally(() => {
				      window.location.href = "/user/payment-failed";
				    }); // Or call a function to show custom UI
		         }
		       },
		       "theme": { "color": "#000000" }
		     };
	 ...
	 function confirmCOD() {
	   Swal.fire({
	 	...
	 	function orderSuccessPopup(){
	 		window.location.href="/user/order-success/page"
	 	}
